...
async def create_software_developer_handler(software_developer_id: str, update_software_developer: UpdateSoftwareDeveloperModel = Body(...), mongo_client: AsyncIOMotorClient = Depends(get_mongo_client), redis_client : redis.Redis = Depends(get_redis_client)) -> JSONResponse:
    try:
        ...
        redis_client.set("software_developer:"f"{software_developer_id}", json.dumps(jsonable_encoder(updated_software_developer)))
