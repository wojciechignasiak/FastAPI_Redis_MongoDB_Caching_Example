...
async def get_software_developer_query(mongo_collection: AsyncIOMotorCollection, sofware_developer_id: str) -> SoftwareDeveloperModel:
    try:
        software_developer: dict = await mongo_collection.find_one({SoftwareDeveloperAttributes.mongo_id: ObjectId(sofware_developer_id)})
        if software_developer is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str({"get_software_developer_query": f"Software developer with id {sofware_developer_id} not found"}))
        else:
